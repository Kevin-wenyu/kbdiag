...
|---|---|---|---|---|---|---|---|
| B1+D1 | `sessions`（D1 由默认列表吸收：只列不是 idle 的客户端会话） | `session.activity` | 04, 10, 11 | 自写，列按 KES V8R6 手册"动态性能视图 4.1 sys_stat_activity"核对（2026-09-23）；SQL 在 `internal/probe/session.go` | `track_activities`、`track_activity_query_size` | 正常 | 已验证 2026-09-26（`e2e/sessions_test.go`、`e2e/commands_test.go`，打磨后的文本断言，node1 主库 + node2 备库） |
| B2 | `session <pid>` | `session.activity`、`lock.list` | 05, 06 | 复用 B1 和 C1 的 SQL，在 `internal/scenario/lock.go` 里按 pid 过滤；2026-09-26 打磨后文本改成键值加 waiting for / blocking / holds 三段 | 同上 | 正常 | 已验证 2026-09-26（`e2e/commands_test.go`，打磨后的文本断言和级别，node1 主库 + node2 备库） |
| C1 | `locks` | `lock.list` | 05, 06, 07, 09 | 自写，`sys_locks` + `sys_blocking_pids()`；SQL 在 `internal/probe/lock.go` | — | 正常 | 已验证 2026-09-26（`e2e/commands_test.go`，打磨后的文本断言和级别，node1 主库 + node2 备库） |
| B4 | `txn` | `session.activity`、`txn.prepared` | 04, 06, 18 | 自写，`sys_prepared_xacts`；SQL 在 `internal/probe/txn.go` | — | 2PC 部分 `not_applicable` | 已验证 2026-09-26（`e2e/commands_test.go`，打磨后的文本断言和级别，node1 主库 + node2 备库） |
| E1 | `waits` | `wait.summary` | 05, 13 | 自写，`sys_stat_activity` 按等待事件和状态聚合；SQL 在 `internal/probe/wait.go` | `track_activities` | 正常 | 已验证 2026-09-26（`e2e/commands_test.go`，打磨后的文本断言和级别，node1 主库 + node2 备库） |
| A1 | `status` | `inst.info`、`inst.downstreams`、`inst.upstream`、`inst.databases`、`inst.disk` | 01, 02, 03 | 自写；SQL 在 `internal/probe/inst.go`；`inst.disk` 不是 SQL，是 statfs | — | 主库上 `inst.upstream` 为 `not_applicable`；远程运行时 `inst.disk` 为 `not_applicable` | `inst.info`、`inst.databases`、`inst.downstreams` 逐行、`inst.upstream`（备库 streaming 和主库 `not_applicable`）、`inst.disk`（socket 读到、TCP 为 `not_applicable`）、连接数用满 FAIL、`kbdiag_ro` 下无遮蔽：已验证 2026-09-26（`e2e/commands_test.go`，node1 主库 + node2 备库）；`inst.upstream` 的 WARN、遮蔽时的 `redacted[]`、`inst.disk` 的 `skipped`/`error`：未验证（只有 L1/L2，见注记） |
| I2 | `slots` | `slot.list` | 16, 22 | 自写，`sys_replication_slots`；SQL 在 `internal/probe/slot.go` | — | 按角色取 LSN | 已验证 2026-09-26（`e2e/commands_test.go`，打磨后的文本断言和级别，node1 主库 + node2 备库） |
...
- C1/B2：`sys_locks` 在 V8R6 上没有 `waitstart` 列，`wait_s` 用等锁会话的 `now()-state_change` 近似，是上限：语句开始后先干了别的再等锁，就会算长一点。L3 用 ksql 在调用前后各取一次 `now()-state_change` 夹住它。被两阶段事务挡住时 `sys_blocking_pids()` 返回 `{0}`，而 prepared 事务的锁行 pid 为 NULL（实测）；这种 blocker 在 finding 里写成"未提交的两阶段事务"，下一步指向 `kbdiag txn`，由 `prepared_waiter.sh` 注入验证（仅主库）。备库上建不了表也拿不到 AccessExclusive，`lock.sh` 在备库改用 advisory lock，所以备库行的 relation 是 NULL。阈值 10 秒用 §6.5 的 A 缩放到 1 秒测。非监控账号看不到别人的 `state_change`，`wait_s` 记进 `redacted[]`，verdict UNKNOWN（L5 一格）。阶段 0 实采：kbdiag_ro 能完整看到 `sys_locks`，只有 `wait_s` 因为要借 activity 的 `state_change` 而被遮蔽。2026-09-26 打磨：文本改成 blockers 段（挡住几个会话、在同一对象上持有什么锁；自己也在排队的写 queued ahead）加 waiting 段（按等待时长排）；advisory 等没有 relation 的锁按锁类型比对象，所以同一挡路者的多个 advisory 锁会一起列出。文本由 L2 golden 覆盖（5 份手排草样来自阶段 0 实采）；VM 上的文本断言（blocker 行第二列是 `1`、session 的 finding 不指向自己）2026-09-26 两节点跑通。2PC 挡路者自己的锁行（pid 为 NULL）阶段 0 没采到，是因为原始 ksql 带了 `pid <> sys_backend_pid()`，把 NULL 也滤掉了；阶段 9 实测它在，node1 上是 `kbdiag_inj_2pc` 的 RowExclusiveLock，文本 holds 显示它，JSON 也保留它（原来被 pid 非空的过滤丢掉，阶段 9 修）
- B4：`txn.long` 的 300 秒和 `txn.prepared` 的 900 秒都用 A（缩放到 1 秒）测；默认阈值下同一注入不出 finding 作为反例。备库上 `txn.prepared` 为 `not_applicable`（主库的 2PC 在备库查不到，实测）。2026-09-26 打磨：两条都只报 WARN，删 `--xact-fail`，`--prepared-fail` 改名 `--prepared-warn`；文本改成 oldest xid（按 2^32 取模比较，列出持有它的会话和 2PC）加 open transactions 加 prepared 三段；被遮蔽的会话 KES 仍给 xid/xmin（阶段 0 实采），所以照样列出、其余列写 `?`；什么都看不到的写 `N sessions hidden`（多半是 idle 会话，不是事务）；有一个来源没采到时 oldest xid 注明"可能有更老的"或写 unknown。文本由 L2 golden 覆盖（6 份手排草样来自阶段 0 实采）；VM 上的文本断言和改级别后的 e2e（WARN、退出码 1，`--xact-fail`/`--prepared-fail` 返回 64）2026-09-26 两节点跑通。主库上最老的 xid 是注入的那个（idle_txn、2PC），walsender 和槽不出现在 txn 里；备库分配不了 xid，列里写 `-`（实测）
- E1：锁等待注入后断言 waiter 落在 `Lock/relation`（备库 `Lock/advisory`）组里；`track_off` 下 `skipped`；`kbdiag_ro` 下三列遮蔽，`rows_affected` 等于遮蔽组的会话总数。只按会话做计数，不判断，所以没有 finding。2026-09-26 打磨：文本只列在干活的组（`Activity` 类等待是进程在主循环里空闲，归后台；state 为空且没有等待事件的也归后台；后台进程卡在其他等待上照样列出，state 写 `(background)`；遮蔽和 untracked 的会话只计数），按会话数、active 优先排，active 却没有等待事件的写 `(running)`，pids 最多列 10 个；idle 和后台进程合成一行 `not shown:` 计数。文本由 L2 golden 覆盖（5 份手排草样来自阶段 0 实采）；VM 上"KSH 进程归后台"的断言 2026-09-26 两节点跑通。阶段 9 发现 `ksh writer` 这类后台进程正在跑查询、又没有等待事件时被当成客户端会话列成 `(running)`；用户选 A：SQL 加一列后台进程 pid（`array_agg ... filter`，排除 client backend、parallel worker 和被遮蔽的行），只给文本用、不进 JSON，2026-09-26 两节点用 system 和 kbdiag_ro 实跑核对过取值（遮蔽行为 NULL）；L2 用阶段 9 实采复原那一刻，e2e 断言后台 pid 不出现在 `(running)` 行（这个竞态很少碰上，e2e 通过不能说明碰上过）；后台进程卡在真正等待上照样列出这一条只有 L2
...
- I2：主库上的槽由 `slot.sh`（暂停备库 walreceiver）造成 inactive；备库上的槽由 `standby_slot.sh` 建，保留的 WAL 必须按 `sys_last_wal_replay_lsn()` 算（备库上 `sys_current_wal_lsn()` 报错），L3 断言它非空且 ≥ 0。2026-09-26 打磨：`slot.inactive` 从 FAIL 改 WARN；symptom 的 WAL 量改用和 pg_size_pretty 一致的单位（原来一律按 MB 取整，45 kB 会写成 0 MB）；逻辑槽的 catalog_xmin 写进 symptom 和 evidence（实验环境 wal_level=replica 建不了逻辑槽，只有 L1/L2）；next 改指 `kbdiag status` 并说"到这个槽的下游节点上运行"；文本按不活跃、保留 WAL 多少排序。文本由 L2 golden 覆盖（5 份手排草样来自阶段 0 实采）；改级别后的 e2e（`slot.inactive` WARN、退出码 1，主库 `slot.sh` 和备库 `standby_slot.sh`）2026-09-26 两节点跑通，`kbdiag_ro` 输出和 system 一致
