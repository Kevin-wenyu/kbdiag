...
- E1：锁等待注入后断言 waiter 落在 `Lock/relation`（备库 `Lock/advisory`）组里；`track_off` 下 `skipped`；`kbdiag_ro` 下三列遮蔽，`rows_affected` 等于遮蔽组的会话总数。只按会话做计数，不判断，所以没有 finding。2026-09-26 打磨：文本只列在干活的组（`Activity` 类等待是进程在主循环里空闲，归后台；state 为空且没有等待事件的也归后台；后台进程卡在其他等待上照样列出，state 写 `(background)`；遮蔽和 untracked 的会话只计数），按会话数、active 优先排，active 却没有等待事件的写 `(running)`，pids 最多列 10 个；idle 和后台进程合成一行 `not shown:` 计数。文本由 L2 golden 覆盖（5 份手排草样来自阶段 0 实采）；VM 上"KSH 进程归后台"的断言 2026-09-26 两节点跑通。已知缺口：`wait.summary` 没有 backend_type，`ksh writer` 这类后台进程正在跑查询、又没有等待事件时会被当成客户端会话列成 `(running)`（阶段 9 node1 实采），怎么改牵涉 JSON 契约，待用户定；后台进程卡在真正等待上照样列出这一条只有 L2
