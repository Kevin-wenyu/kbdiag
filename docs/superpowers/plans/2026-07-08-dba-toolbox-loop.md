...
| 6 | audit 补充:连接来源白名单(sys_hba.conf 分析) | DBA | ✅ 2026-07-09 迭代 7 |
| 7 | 次要消重遗留:checkpoint/bgwriter 数据源、ANALYZE drift 公式 | 工程 | ✅ 2026-07-09 迭代 8 |
...
### 迭代 7 — 2026-07-09:audit 连接来源白名单(任务 #6,commit c7872e9)

- `_audit_hba()`:查 `sys_hba_file_rules` 视图(服务器实际加载的规则,含
  parse error 列;实测普通用户也可读)而非解析 sys_hba.conf 文件——磁盘
  文件在 reload 前可能与生效配置不一致。视图不可用时降级为提示人工检查。
- 风险模型四类:trust 认证(local/网络分开措辞)、`0.0.0.0/0`/`::/0`/`all`
  全网开放(replication 通道单独点名)、弱认证方法(password/md5)、parse
  error。单条 SQL 用 concat_ws 拼 per-rule 风险原因,超过 TOP_N 截断。
- 新发现 KingbaseES 特有行为(已记入 CLAUDE.md):Oracle 模式下 `||` 拼接
  把 NULL 当空串,`NULL || '/'` 得 `'/'`,`coalesce(a||b, fallback)` 兜底
  永远不触发,必须用 CASE 显式判断。首版 local 规则 source 列因此显示成
  `/`,已修。
- 测试 +3(hba 段存在、全网开放规则被标记、trust 规则被标记),audit 套件
  10/10;perf audit 1387ms/20s 预算内。

### 迭代 8 — 2026-07-09:次要消重(任务 #7,commit 9352834)

- `core.sh` 新增共享数据源段:`bgwriter_stats()`(单条 SQL 返回
  sys_stat_bgwriter 全部消费字段+pg_size_pretty 列,`perf wal` 与 `stat`
  checkpoint 段各取所需粒度)和 `KB_DRIFT_EXPR`(ANALYZE drift 公式片段,
  advisor 全库巡检与 colstat 单表深挖同口径;colstat 删掉 awk 重算,改在
  SQL 内算)。
- 消重过程中实锤一个存量真 bug:advisor `_advisor_analyze` 的查询因
  `relname` 歧义(join sys_class 后未加表别名)每次执行都报错,被
  `2>/dev/null || true` 吞掉——ANALYZE 建议自上线以来从未生效过。加
  `t.`/`sc.` 别名修复。教训与迭代 2.5/3 同类:错误吞噬(`2>/dev/null`)
  会把结构性故障伪装成"没有发现问题"。
- advisor 套件存量 3 失败(HEAD 复测确认与本轮无关)根因是测试自身写错:
  `grep -i 'ANALYZE' | head -1` 永远先命中 `==> Advisor: Analyze` 标题行;
  index 用例更是"抓 CREATE INDEX 行却断言以 DROP INDEX 开头"。改为只匹配
  语句行(`^VACUUM `/`^ANALYZE `/`^(CREATE|DROP) INDEX `)并断言语句完整。
- 门禁:perf 28/28、stat 15/15、advisor 20/20、colstat 13/13,全量 perf
  check 通过(stat 10358ms/20s 最接近预算,与本轮无关,系 Top SQL 段)。
- 任务队列 #0–#8 至此全部完成,spec 2026-07-04 的消重遗留清零。
