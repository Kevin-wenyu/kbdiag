...
func TestExitCodesWithoutDatabase(t *testing.T) {
	...
	cases := []struct {
		...
		{"conn-fail below conn-warn", []string{"status", "--conn-warn", "90", "--conn-fail", "80"}, 64, false},
		{"conn-warn not a number", []string{"status", "--conn-warn", "x"}, 64, false},
		{"conn-warn NaN", []string{"status", "--conn-warn", "NaN"}, 64, false},
		{"conn-fail NaN", []string{"status", "--conn-fail", "NaN"}, 64, false},
		{"conn-fail Inf", []string{"status", "--conn-fail", "Inf"}, 64, false},
		{"conn-warn zero", []string{"status", "--conn-warn", "0"}, 64, false},
		{"conn-warn negative", []string{"status", "--conn-warn=-10", "--conn-fail", "50"}, 64, false},
		{"status with conn thresholds cannot connect", []string{"status", "--conn-warn", "50", "--conn-fail", "50", "--host", sock}, 69, false},
