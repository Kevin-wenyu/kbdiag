...
func TestExitCodesWithoutDatabase(t *testing.T) {
	...
	cases := []struct {
		...
		{"txn --xact-fail is gone", []string{"txn", "--xact-fail", "60"}, 64, false},
		{"txn --prepared-fail is gone", []string{"txn", "--prepared-fail", "60"}, 64, false},
		{"txn --prepared-warn cannot connect", []string{"txn", "--prepared-warn", "60", "--host", sock}, 69, false},
