...
`docs/agents/` 是给 agent 技能读的配置说明，不算活文档。配套的用户手册在独立仓库 `kbdiag-docs`（Hugo 站点，中英双语），归它自己的仓库管，也不算本仓库的活文档：v2 的页面写在它的 `v2` 分支上，每条命令过了 VM 测试再写，示例用 VM 实跑结果并注明 Go commit；发布 `v2.0.0-alpha.1` 时合进它的 main，在这之前线上站点保持 v1 手册。`AGENTS.md` 是 Codex 的入口，只指向本文件，不算活文档。
