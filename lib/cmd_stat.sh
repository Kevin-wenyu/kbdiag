...
cmd_stat() {
  ...
  # a separate sleep per section doubled stat's runtime.
  local snap1 snap2
  while [[ $i -lt $count ]]; do
  ...
  _stat_section_temp_files "$interval" "$snap1" "$snap2"
...
# Consumes the throughput snapshot pair (fields 6-7: temp_files, temp_bytes)
# instead of sleeping through its own sampling window.
_stat_section_temp_files() {
  local interval="${1:-5}" s1="$2" s2="$3"
  printf "\nTemp files (%ss delta):\n" "$interval"
